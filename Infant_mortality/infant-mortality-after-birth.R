...
infant_2020 <- read_delim(paste0(file_path, "Linked Birth  Infant Death Records, 2017-2020 Expanded.txt"), delim = "\t")

# Rename columns
colnames(infant_2020) <- c("Notes", "Age_days", "Age_code", "Deaths", "Births", "Death_rate")
...
ggsave(paste0(file_path, "daily_infant_mortality_US.png"))
...
ggplot(infant_2020, aes(x = Age_code, y = Cumulative_deaths / Births)) +
  geom_line(color = "#b16214", size = 1.5) +
  ...
  labs(title = "Share of infants who have died over the first year",
       caption = "The cumulative share of infants who have died by a given age.\nBased on US infant mortality rates between 2017-2020, using death certificates.\nSource: US Centers for Disease Control and Prevention.",
       x = "Age (days)",
       y = "") +
  theme_minimal()

ggsave(paste0(file_path, "cumulative_infant_mortality_US.svg"))
